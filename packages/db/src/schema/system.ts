...
export const systemHealthLogs = pgTable('system_health_logs', {
  ...
  index('idx_system_health_logs_created_at').using('btree', systemHealthLog.createdAt.asc().nullsLast().op('timestamp_ops')),
  index('idx_system_health_logs_status').using('btree', systemHealthLog.status.asc().nullsLast().op('enum_ops'))
])
...
export const errorLogs = pgTable('error_logs', {
  ...
  index('idx_error_logs_created_at').using('btree', errorLog.createdAt.asc().nullsLast().op('timestamp_ops')),
  index('idx_error_logs_level').using('btree', errorLog.level.asc().nullsLast().op('text_ops')),
  index('idx_error_logs_resolved').using('btree', errorLog.resolved.asc().nullsLast().op('bool_ops')),
  foreignKey({
...
export const siteConfig = pgTable('site_config', {
  ...
  index('idx_site_config_type').using('btree', siteConfigRow.type.asc().nullsLast().op('enum_ops')),
  foreignKey({
...
export const bulkOperations = pgTable('bulk_operations', {
  ...
  index('idx_bulk_operations_created_at').using('btree', bulkOperation.createdAt.asc().nullsLast().op('timestamp_ops')),
  index('idx_bulk_operations_status').using('btree', bulkOperation.status.asc().nullsLast().op('enum_ops')),
  foreignKey({
