      ...
      <Card>
        ...
        <CardContent>
          <div className='flex items-center space-x-4'>
            <div className='bg-bg-hover h-16 w-16 animate-pulse rounded-lg' />
  ...
  if (!currentTrack) {
    return (
      <Card>
        ...
        <CardContent>
          <div className='flex items-center justify-between'>
            <p className='text-text-secondary'>{t('spotify.now-listening.not-playing')}</p>
            <button
              type='button'
              onClick={handleRefresh}
              disabled={isRefreshing}
    ...
    <Card>
      ...
      <CardContent>
        <Link
          ...
          className='hover:bg-bg-hover group flex items-center space-x-4 rounded-lg p-4 transition-colors'
        >
          <div className='relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg'>
            <MusicImage
              src={currentTrack.albumImage}
              alt={`${currentTrack.album} album cover`}
              fallbackIcon={<PlayIcon className='text-text-secondary h-6 w-6' />}
              width={64}
              height={64}
              sizes='64px'
            />
            {currentTrack.isPlaying && (
              <div className='bg-bg-base/20 absolute inset-0 flex items-center justify-center'>
          ...
          <div className='flex-shrink-0'>
            <div className='relative flex h-6 w-6 items-center justify-center'>
              {currentTrack.isPlaying ? (
                ...
                <Music className='text-text-secondary h-5 w-5 opacity-50' />
              )}
            </div>
          </div>
        </Link>
