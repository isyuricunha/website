...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (available_providers.length === 0) {
            return NextResponse.json({ error: 'No AI providers are currently available.' }, { status: 503 })
        }
        ...
        const provider = requested_provider && available_providers.includes(requested_provider)
            ? requested_provider
            : default_provider
