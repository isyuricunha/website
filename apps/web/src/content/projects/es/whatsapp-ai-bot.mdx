...
Este repositorio incluye un _hook_ opcional _pre-commit_ que ejecuta un escaneo ligero de secretos en los archivos preparados (_staged_).

```bash
pnpm run hooks:install
```

Si se detecta un patrón similar a un secreto (por ejemplo, `MISTRAL_API_KEY=...`), el _commit_ será bloqueado.

---

### CI

GitHub Actions se ejecuta en cada _Pull Request_ y en los _pushes_ a `main`:
