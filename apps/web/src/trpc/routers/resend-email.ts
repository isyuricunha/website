...
export const resendEmailRouter = createTRPCRouter({
  ...
  getAudiences: adminProcedure.query(async () => {
    try {
  ...
  getEmailStats: adminProcedure.query(async ({ ctx }) => {
    try {
      ...
      return {
        ...
        subscribers: {
          ...
          subscriptionRate:
            totalUsers.length > 0
              ? Math.round((totalSubscribers.length / totalUsers.length) * 100)
              : 0
        }
      }
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        subscribers: {
          ...
          totalUsers,
          subscriptionRate:
            totalUsers > 0 ? Math.round((realSubscriberCount / totalUsers) * 100) : 0
        }
      }
    } catch (error) {
  ...
  subscribeUser: protectedProcedure.mutation(async ({ ctx }) => {
    try {
  ...
  unsubscribeUser: protectedProcedure.mutation(async ({ ctx }) => {
    try {
